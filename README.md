...
Options:
  ...
  --declaration-map               Write an adjacent .d.ts.map for each generated declaration bundle
                                                                          [boolean] [default: false]
  --declaration-map-inline-sources Include original source content in the map; requires
                                  --declaration-map                       [boolean] [default: false]
  --declaration-map-source-root    Set the map's sourceRoot; requires --declaration-map      [string]
...
To generate a declaration map for Go-to-Definition into the original TypeScript source:

```bash
./node_modules/.bin/dts-bundle-generator --declaration-map -o dist/index.d.ts src/index.ts
```

This writes `dist/index.d.ts.map` alongside the declaration bundle and adds a `sourceMappingURL` comment. Map generation is opt-in. Library callers can use `generateDtsBundleWithMaps` to receive declaration and map text without writing files; `generateDtsBundle` continues returning declaration strings.
