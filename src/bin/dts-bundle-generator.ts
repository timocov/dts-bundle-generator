...
import { generateDtsBundleWithMaps } from '../bundle-generator';
...
interface ParsedArgs extends yargs.Arguments {
	...
	'declaration-map': boolean;
	'declaration-map-inline-sources': boolean;

	'out-file': string | undefined;
	'umd-module-name': string | undefined;
	project: string | undefined;
	config: string | undefined;
	'declaration-map-source-root': string | undefined;
...
function parseArgs(): ParsedArgs {
	return yargs
		...
		.option('declaration-map', {
			type: 'boolean',
			default: false,
			description: 'Emit an adjacent Source Map v3 file for each generated declaration bundle',
		})
		.option('declaration-map-inline-sources', {
			type: 'boolean',
			default: false,
			description: 'Include original source content in declaration maps (requires --declaration-map)',
		})
		.option('declaration-map-source-root', {
			type: 'string',
			description: 'Set sourceRoot in declaration maps (requires --declaration-map)',
		})
		.version()
		.strict()
...
function main(): void {
			...
			entries: args._.map<ConfigEntryPoint>((entryPath: string | number) => {
				return {
					...
					output: {
						...
						declarationMap: args['declaration-map'],
						declarationMapInlineSources: args['declaration-map-inline-sources'],
						declarationMapSourceRoot: args['declaration-map-source-root'],
					},
	...
	const outputFileNames = bundlerConfig.entries.map(entry => entry.outFile !== undefined ? entry.outFile : generateOutFileName(entry.filePath));
	const generatedDts = generateDtsBundleWithMaps(bundlerConfig.entries, bundlerConfig.compilationOptions, outputFileNames);

	const outFilesToCheck: string[] = [];
	for (let i = 0; i < bundlerConfig.entries.length; ++i) {
		const entry = bundlerConfig.entries[i];
		const outFile = outputFileNames[i];

		normalLog(`Writing ${entry.filePath} -> ${outFile}`);
		ts.sys.writeFile(outFile, generatedDts[i].declarationText);
		const declarationMap = generatedDts[i].declarationMap;
		if (declarationMap !== null) {
			ts.sys.writeFile(`${outFile}.map`, declarationMap);
		}
