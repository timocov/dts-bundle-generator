...
export interface OutputStatementPosition {
...
export interface GeneratedOutput {
	text: string;
	statementPositions: OutputStatementPosition[];
}

export function generateOutput(params: OutputParams, options: OutputOptions = {}): string {
	return generateOutputWithStatementPositions(params, options, false).text;
}

// eslint-disable-next-line complexity
export function generateOutputWithStatementPositions(params: OutputParams, options: OutputOptions = {}, trackStatementPositions: boolean = true): GeneratedOutput {
	const resultOutputParts: { text: string; statements?: StatementText[] }[] = [];

	if (!options.noBanner) {
		resultOutputParts.push({ text: `// Generated by dts-bundle-generator v${packageVersion()}` });
	}

	if (params.typesReferences.size !== 0) {
		const header = generateReferenceTypesDirective(Array.from(params.typesReferences));
		resultOutputParts.push({ text: header });
	}
	...
	if (params.imports.size !== 0) {
		...
		if (importsArray.length !== 0) {
			resultOutputParts.push({ text: importsArray.join('\n') });
		}
	}
	...
	if (statements.length !== 0) {
		resultOutputParts.push({ text: statementsTextToString(statements), statements });
	}

	if (params.wrappedNamespaces.size !== 0) {
		resultOutputParts.push({ text:
	...
	if (params.renamedExports.size !== 0) {
		resultOutputParts.push({ text: `export {\n\t${
	...
	if (options.umdModuleName !== undefined) {
		resultOutputParts.push({ text: `export as namespace ${options.umdModuleName};` });
	}
	...
	const statementPositions: OutputStatementPosition[] = [];
	let lineOffset = 0;
	for (const part of resultOutputParts) {
		if (trackStatementPositions && part.statements !== undefined) {
			const sourceFile = ts.createSourceFile('output.d.ts', part.text, ts.ScriptTarget.Latest, false, ts.ScriptKind.TS);
			for (let index = 0; index < part.statements.length; index += 1) {
				const outputStatement = sourceFile.statements[index];
				if (outputStatement === undefined) {
					continue;
				}
				const position = sourceFile.getLineAndCharacterOfPosition(outputStatement.getStart(sourceFile));
				statementPositions.push({
					statement: part.statements[index].statement,
					line: lineOffset + position.line,
					column: position.character,
					text: outputStatement.getText(sourceFile),
				});
			}
		}
		if (trackStatementPositions) {
			lineOffset += countLines(part.text) + 2;
		}
	}

	return { text: resultOutputParts.map(part => part.text).join('\n\n'), statementPositions };
}
...
function getStatementText(statement: ts.Statement, includeSortingValue: boolean, helpers: OutputHelpers): StatementText {
	...
	return { text: statementText, sortingValue, statement };
}
